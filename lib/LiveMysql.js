...
function LiveMysql(settings, callback) {
  ...
  if (settings.pool) {
    ...
    self.escape = self.pool.escape.bind(self.pool);
    self.escapeId = self.pool.escapeId;
    self.possiblyPooledQuery = self.pool.query.bind(self.pool);
    ...
    self.escape = self.db.escape.bind(self.db);
    self.escapeId = self.db.escapeId;
    self.possiblyPooledQuery = self.db.query.bind(self.db);
    self.endDbOrPool = function() {
      self.db.destroy();
    };
    initialConnect = self.db.connect;
  }
